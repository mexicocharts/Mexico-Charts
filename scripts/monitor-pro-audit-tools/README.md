...
`roster-probe-batch.mjs` now reserves a fixed 120-second complete driver lifecycle plus 5 seconds for durable checkpointing inside each unchanged 150-second batch. It checks admission both before and after persisting an attempt. It never passes the driver the shrinking tail of a previous batch. Existing 30-second statements, 32-second query deadlines, connection guards and read-only checks remain unchanged.

`roster-attempt-store.mjs` uses exclusive, fsynced immutable claims/outcomes and a process lock. Resume reconstructs progress from verified saved results and attempt outcomes, not a lossy in-memory cursor. Incomplete evaluations remain completed *evaluations* with unresolved readiness. Proven zero-query deferrals retain the same next artist; ambiguous in-flight evidence stops rather than repeating a request. Artist query timeouts remain capture failures for later review; other driver failures stop for systemic review.

`roster-resume-runner.mjs` uses the unchanged pinned configured driver/evaluator and the already reconciled selection. It creates a separate continuation root containing byte-exact source-proof copies with explicit lineage, without recapturing the population. The original 19 results and the zero-query failure remain untouched. The initial failed artist receives a new, explicitly separate evidence namespace. Later proven zero-query deferrals can use a new generation, while attempted source queries are never automatically repeated. Runner/helper hashes, snapshot hashes, selection and baseline hashes are pinned in the durable lineage.

Use `--base /private/audit/base --action verify` for source/lineage verification without database access; `--action run --batches N` executes bounded batches (maximum 200). `status` and `export` inspect retained decisions only. An ambiguous interrupted attempt or stale driver lock requires integrity review; never delete old reservations to force resume. Tests cover budget admission, partial completion, repeated resume, zero-query deferrals, exclusive persistence, scope drift and concurrency. Production execution remains a separate validation step.

### Catalog/artwork reconciliation (job 1 only)

`reconcile-captured-catalog.py` accounts for physical rows in already-captured Kworb HTML, verifies the original representation hash/size and source date, and preserves unknown numeric readings. Its completeness scope is the captured source table, not every Spotify object. `catalog-evidence-reconciliation.mjs` creates a per-candidate manifest of partial licensed items, cached artwork and identity-bound complete captures. It never treats partial lists as a full denominator, certifies artwork enrichment, rewrites acquisition dates, or resolves identity conflicts.
