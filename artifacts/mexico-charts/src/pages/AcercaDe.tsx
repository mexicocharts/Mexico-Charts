...
export default function AcercaDe() {
  ...
  const PILLARS = [
    { title: pick("Datos", "Data"), body: pick("Rankings, señales de streaming, fuentes públicas y lectura editorial para entender movimiento real.", "Rankings, streaming signals, public sources and editorial analysis to understand real movement."), icon: BarChart3 },
    { title: pick("Cultura", "Culture"), body: pick("Artistas, escenas, géneros, comunidades y momentos que empujan la música mexicana.", "Artists, scenes, genres, communities and moments moving Mexican music forward."), icon: Sparkles },
    { title: pick("Industria", "Industry"), body: pick("Certificaciones, mercado, giras y contexto para leer el impacto más allá de una lista.", "Certifications, market, touring and context to understand impact beyond a chart."), icon: Landmark },
  ];
  const COVERAGE = [
    { label: pick("Listas", "Charts"), detail: pick("Plataformas digitales", "Digital platforms"), icon: Disc3 },
    { label: pick("Artistas", "Artists"), detail: pick("Perfiles y audiencia", "Profiles and audience"), icon: Users },
    { label: "Streaming", detail: pick("Crecimiento y señales", "Growth and signals"), icon: Radio },
    { label: pick("Industria", "Industry"), detail: pick("Mercado y contexto", "Market and context"), icon: Globe2 },
  ];
  const aboutJsonLd = {
    "@context": "https://schema.org",
    "@type": "AboutPage",
    name: pick("Acerca de Mexico Charts", "About Mexico Charts"),
  ...
  return (
    <div style={{ background: "#080808", minHeight: "100vh", color: "#fff", overflowX: "hidden" }}>
      <PageSEO
        title={pick("Acerca de Mexico Charts — Datos y cultura de la música mexicana", "About Mexico Charts — Mexican music data and culture")}
        description={pick("Mexico Charts es una plataforma independiente de datos sobre música mexicana, listas, artistas, streaming, industria, certificaciones y giras.", "Mexico Charts is an independent data platform covering Mexican music, charts, artists, streaming, industry, certifications and touring.")}
        path="/acerca-de"
        jsonLd={aboutJsonLd}
      />
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-6 py-14 sm:py-18 lg:px-10 lg:py-24">
        <EditorialHero
          title={pick("Datos con contexto cultural", "Data with cultural context")}
          description={pick("Mexico Charts documenta el crecimiento, desempeño e impacto de la música mexicana dentro de México y alrededor del mundo.", "Mexico Charts documents the growth, performance and impact of Mexican music in Mexico and around the world.")}
          aside={<div
          ...
          >
            ...
            <h2 className="text-sm font-black uppercase tracking-[0.16em] text-white">{pick("Independiente", "Independent")}</h2>
            <p className="mt-4 text-sm font-medium leading-7 text-zinc-500">
              {pick("No somos una lista oficial de una plataforma. Reunimos señales disponibles y criterio editorial para hacer más legible el momento de la música mexicana.", "We are not an official chart from any one platform. We combine available signals and editorial judgment to make the current state of Mexican music easier to understand.")}
            </p>
          </div>}
        />
        ...
        <section className="mt-14 grid gap-8 lg:grid-cols-[360px_minmax(0,1fr)] lg:items-start">
          <div>
            <h2 className="text-sm font-black uppercase tracking-[0.22em]" style={{ color: G }}>{pick("Qué cubrimos", "What we cover")}</h2>
            <p className="mt-4 text-sm font-medium leading-7 text-zinc-500">
              {pick("Listas, artistas, streaming, giras, industria y tendencias culturales en un solo espacio diseñado para lectura rápida.", "Charts, artists, streaming, touring, industry and cultural trends in one space designed for quick reading.")}
            </p>
          </div>
        ...
        >
          <h2 className="text-[10px] font-black uppercase tracking-[0.22em]" style={{ color: "rgba(255,255,255,0.38)" }}>{pick("Aviso independiente", "Independence notice")}</h2>
          <p className="mt-3 text-xs font-medium leading-6" style={{ color: "rgba(255,255,255,0.36)" }}>
            {pick("Mexico Charts es una plataforma independiente. No estamos afiliados oficialmente con Spotify, YouTube, Apple Music, Deezer, Billboard, IFPI, AMPROFON, Pollstar, Ticketmaster ni con ningún artista, sello discográfico o plataforma mencionada, salvo que se indique lo contrario.", "Mexico Charts is an independent platform. We are not officially affiliated with Spotify, YouTube, Apple Music, Deezer, Billboard, IFPI, AMPROFON, Pollstar, Ticketmaster, or any artist, record label or platform mentioned unless explicitly stated.")}
          </p>
        </section>
        ...
        <div className="mt-10 flex flex-wrap gap-3">
          <Link
            ...
            style={{ background: G }}
          >
            {pick("Explorar MX100", "Explore MX100")}
          </Link>
          <Link
            href="/charts"
