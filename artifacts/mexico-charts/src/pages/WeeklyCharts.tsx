...
export default function WeeklyCharts() {
  ...
  const archiveEdition = routeDate ? weeklyEdition(routeDate) : null;
  const requestedDate = archiveEdition?.date ?? null;
  const invalidHistoricalRoute = routeDate !== null && archiveEdition === null;
  const archiveNeighbors = requestedDate ? weeklyEditionNeighbors(requestedDate) : { newer: null, older: null };
  ...
  const hub = useQuery<Hub>({ queryKey: ["charts-hub"], queryFn: async () => { const r = await fetch("/api/charts/hub", { cache: "no-store" }); if (!r.ok) throw new Error(); return r.json(); }, staleTime: 60_000, enabled: !invalidHistoricalRoute });
  const weekly = useQuery<Summary>({ queryKey: ["weekly-editorial", requestedDate], queryFn: async () => { const r = await fetch(`/api/charts/editorial/weekly${requestedDate ? `?date=${requestedDate}` : ""}`); if (!r.ok) throw new Error(); return r.json(); }, staleTime: 15 * 60_000, enabled: !invalidHistoricalRoute });
  ...
  const seoPath = requestedDate ? `/esta-semana/${requestedDate}` : "/esta-semana";
  const seoTitle = requestedDate
    ? pick(`Esta semana — edición ${formatDate(requestedDate)} | Mexico Charts`, `This week — ${formatDate(requestedDate)} edition | Mexico Charts`)
    : pick("Esta semana — Mexico Charts", "This week — Mexico Charts");
  const seoDescription = requestedDate
    ? pick(`Participación mexicana en los charts semanales de Spotify y YouTube para México, edición ${formatDate(requestedDate)}.`, `Mexican participation in Spotify and YouTube weekly charts for Mexico, ${formatDate(requestedDate)} edition.`)
    : pick("Participación mexicana verificada en los charts semanales de Spotify y YouTube para México.", "Verified Mexican participation in Spotify and YouTube weekly charts for Mexico.");
  const pageSchema = archiveEdition ? {
    "@type": "WebPage",
    dateModified: archiveEdition.updatedAt,
    temporalCoverage: archiveEdition.date,
  } : undefined;

  return <div className="min-h-screen overflow-x-clip bg-[#050505] text-white"><PageSEO title={seoTitle} description={seoDescription} path={seoPath} noindex={invalidHistoricalRoute} jsonLd={pageSchema} breadcrumbs={[
    { name: "Mexico Charts", path: "/" },
    { name: pick("Charts de música en México", "Mexico music charts"), path: "/charts" },
    ...(requestedDate ? [{ name: pick("Esta semana", "This week"), path: "/esta-semana" }] : []),
    { name: requestedDate ? `${pick("Edición", "Edition")} ${requestedDate}` : pick("Esta semana", "This week"), path: requestedDate ? `/esta-semana/${requestedDate}` : "/esta-semana" },
  ]} /><SiteNav />
    <main className="mx-auto max-w-[1380px] px-4 py-7 sm:px-6 lg:px-10 lg:py-12">
      {invalidHistoricalRoute ? <section className="border border-white/10 bg-[#090909] px-5 py-12 sm:px-8"><p className="text-[9px] font-black uppercase tracking-[.28em]" style={{ color: G }}>Mexico Charts · Archivo semanal</p><h1 className="mt-6 text-4xl font-black uppercase tracking-[-.04em]">{pick("Edición no disponible", "Edition unavailable")}</h1><p className="mt-4 max-w-2xl text-sm leading-relaxed text-zinc-400">{pick("Esta fecha no corresponde a una edición histórica verificada. Consulta la edición actual o el archivo disponible.", "This date does not match a verified historical edition. View the current edition or the available archive.")}</p><Link href="/esta-semana"><span className="mt-7 inline-block text-[10px] font-black uppercase tracking-[.16em]" style={{ color: G }}>{pick("Volver a esta semana", "Return to this week")} →</span></Link></section> : <>
      <header className="relative overflow-hidden border border-white/10 bg-[#090909] px-5 py-7 sm:px-8 lg:px-10"><div className="pointer-events-none absolute inset-y-0 right-0 w-2/3" style={{ background: "radial-gradient(circle at 85% 20%, rgba(57,255,20,.13), transparent 58%)" }} /><div className="relative flex flex-col justify-between gap-7 lg:flex-row lg:items-end"><div><div className="flex items-center gap-3"><img src={`${import.meta.env.BASE_URL}mexico-charts-logo.png`} alt="Mexico Charts" className="h-9 w-9 object-contain" /><span className="text-[9px] font-black uppercase tracking-[.28em]" style={{ color: G }}>Mexico Charts · Editorial</span></div><h1 className="mt-7 text-[clamp(3.1rem,7vw,6.5rem)] font-black uppercase leading-[.84] tracking-[-.065em]">Esta <span style={{ color: G }}>semana</span></h1><p className="mt-5 max-w-2xl text-sm leading-relaxed text-zinc-400 sm:text-base">{pick("Una lectura semanal de la participación mexicana en los rankings oficiales de Spotify y YouTube para México, con Apple Music y Deezer como contexto cuando hay ediciones guardadas.", "A weekly view of Mexican participation in official Spotify and YouTube rankings for Mexico, with Apple Music and Deezer context when saved editions are available.")} <Link href="/charts" className="font-bold text-zinc-300 underline decoration-white/20 underline-offset-4">{pick("Ver todos los charts de música en México.", "View all Mexico music charts.")}</Link></p></div><div className="border border-white/10 bg-black/40 p-5"><div className="text-[9px] font-black uppercase tracking-[.2em] text-zinc-500">{pick("Edición", "Edition")}</div><time dateTime={editionDate?.slice(0, 10)} className="mt-2 block text-xl font-black uppercase">{formatDate(editionDate)}</time><div className="mt-4 flex flex-wrap gap-x-4 gap-y-2">{archiveNeighbors.older && <Link href={`/esta-semana/${archiveNeighbors.older.date}`}><span className="text-[9px] font-black uppercase tracking-[.16em]" style={{ color: G }}>← {pick("Anterior", "Older")}</span></Link>}{archiveNeighbors.newer && <Link href={`/esta-semana/${archiveNeighbors.newer.date}`}><span className="text-[9px] font-black uppercase tracking-[.16em]" style={{ color: G }}>{pick("Siguiente", "Newer")} →</span></Link>}</div></div></div></header>
      <nav aria-label={pick("Archivo de ediciones semanales", "Weekly edition archive")} className="my-4 border border-white/10 bg-[#090909] px-4 py-4 sm:px-6"><div className="text-[9px] font-black uppercase tracking-[.2em] text-zinc-500">{pick("Archivo semanal", "Weekly archive")}</div><div className="mt-3 flex flex-wrap gap-2"><Link href="/esta-semana"><span className={`inline-block border px-3 py-2 text-[9px] font-black uppercase tracking-[.14em] ${requestedDate ? "border-white/10 text-zinc-500" : "border-[#39FF14]/50 text-[#39FF14]"}`}>{pick("Edición actual", "Current edition")}</span></Link>{WEEKLY_EDITIONS.map(item => <Link key={item.date} href={`/esta-semana/${item.date}`}><time dateTime={item.date} className={`inline-block border px-3 py-2 text-[9px] font-black uppercase tracking-[.14em] ${requestedDate === item.date ? "border-[#39FF14]/50 text-[#39FF14]" : "border-white/10 text-zinc-500"}`}>{formatDate(item.date)}</time></Link>)}</div></nav>
