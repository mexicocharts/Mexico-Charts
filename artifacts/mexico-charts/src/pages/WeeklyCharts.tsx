...
export default function WeeklyCharts() {
  ...
  async function downloadCard() { if (!cardRef.current || !leader) return; setExporting(true); try { const href = await toPng(cardRef.current, { pixelRatio: 2, cacheBust: true, backgroundColor: "#070707" }); const a = document.createElement("a"); a.download = `mexico-charts-${platformId}-${activeMode}-${editionDate ?? "actual"}.png`; a.href = href; a.click(); } finally { setExporting(false); } }
