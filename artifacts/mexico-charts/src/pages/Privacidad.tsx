...
import { BarChart3, ChevronRight, Cookie, CreditCard, ExternalLink, Home, Mail, MousePointer2, ShieldCheck, UserRound, Youtube } from "lucide-react";
...
export default function Privacidad() {
  ...
  const privacyItems = [
    ...
    { title: pick("Servicios de terceros", "Third-party services"), body: pick("Mexico Charts integra o enlaza servicios externos, entre ellos YouTube, Spotify, Apple Music, Deezer, Instagram, TikTok, X y Ticketmaster. Cada servicio aplica sus propios términos y políticas. El tratamiento específico de los servicios de la API de YouTube se explica en la sección correspondiente de esta política.", "Mexico Charts integrates or links to external services including YouTube, Spotify, Apple Music, Deezer, Instagram, TikTok, X and Ticketmaster. Each service applies its own terms and policies. Our specific handling of YouTube API Services is explained in the corresponding section of this policy."), icon: ExternalLink },
  ...
  return (
    <div style={{ background: "#080808", minHeight: "100vh", color: "#fff", overflowX: "hidden" }}>
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-6 py-14 sm:py-18 lg:px-10 lg:py-24">
        ...
        <section className="mt-14 rounded-2xl border border-white/[0.075] bg-white/[0.028] p-5 sm:p-7" aria-labelledby="youtube-api-privacy">
          <div className="mb-5 flex h-10 w-10 items-center justify-center rounded-xl bg-red-500/10 text-red-400"><Youtube className="h-5 w-5" /></div>
          <h2 id="youtube-api-privacy" className="text-xs font-black uppercase tracking-[0.18em]" style={{ color: G }}>{pick("Servicios de la API de YouTube", "YouTube API Services")}</h2>
          <div className="mt-4 space-y-4 text-sm font-medium leading-7 text-zinc-500">
            <p>{pick("Mexico Charts utiliza los servicios de la API de YouTube para localizar y mostrar información pública relacionada con artistas, canales, listas de reproducción y videos. Los datos que podemos obtener incluyen identificadores de videos y canales; títulos; nombres de canales; fechas de publicación; miniaturas; duración; relaciones con listas públicas; y estadísticas públicas como vistas, Me gusta, comentarios, suscriptores y número de videos, cuando YouTube las proporciona.", "Mexico Charts uses YouTube API Services to locate and display public information related to artists, channels, playlists and videos. Data we may retrieve includes video and channel identifiers; titles; channel names; publication dates; thumbnails; duration; public-playlist relationships; and public statistics such as views, likes, comments, subscribers and video counts when YouTube provides them.")}</p>
            <p>{pick("Usamos esta información para verificar relaciones entre artistas y canales, mantener un catálogo público de videos, mostrar la observación más reciente disponible y crear análisis independientes de Mexico Charts, como cambios entre observaciones, ganancias por periodo, porcentajes, velocidad, comparaciones y rankings. Los totales atribuidos a YouTube se identifican como observaciones de YouTube Data API; los cambios, tendencias, estimaciones y rankings se identifican como cálculos de Mexico Charts.", "We use this information to verify artist and channel relationships, maintain a public video catalog, display the latest available observation, and create independent Mexico Charts analytics such as changes between observations, period gains, percentages, velocity, comparisons and rankings. Totals attributed to YouTube are identified as YouTube Data API observations; changes, trends, estimates and rankings are identified as Mexico Charts calculations.")}</p>
            <p>{pick("Esta integración consulta datos públicos con una credencial de servidor. En la funcionalidad descrita aquí, Mexico Charts no solicita acceso a una cuenta personal de YouTube ni utiliza autorización OAuth del usuario.", "This integration queries public data with a server credential. For the functionality described here, Mexico Charts does not request access to a user's personal YouTube account and does not use end-user YouTube OAuth authorization.")}</p>
            <p>{pick("Mexico Charts conserva los datos obtenidos mediante los servicios de la API de YouTube únicamente durante el tiempo permitido y necesario para prestar estas funciones. Los datos públicos que pueden cambiar se actualizan o eliminan dentro de los plazos aplicables, y procuramos mostrar la información más reciente disponible. Las observaciones estadísticas históricas y los cálculos de Mexico Charts se conservan solamente durante los periodos permitidos por las políticas aplicables y por cualquier autorización escrita que corresponda. Una ampliación del periodo de almacenamiento o del uso de métricas derivadas se aplicará únicamente después de recibir la autorización correspondiente de Google.", "Mexico Charts retains data obtained through YouTube API Services only for the time permitted and necessary to provide these features. Public data that may change is refreshed or deleted within the applicable periods, and we seek to display the most recent information available. Historical statistical observations and Mexico Charts calculations are retained only for periods permitted by the applicable policies and any relevant written authorization. Any extended storage period or expanded use of derived metrics will apply only after the corresponding authorization is received from Google.")}</p>
            <p>{pick("El uso de las funciones de YouTube está sujeto a los ", "Use of YouTube features is subject to the ")}<a className="text-white underline underline-offset-4" href="https://www.youtube.com/t/terms" target="_blank" rel="noreferrer">{pick("Términos de Servicio de YouTube", "YouTube Terms of Service")}</a>{pick(", al ", ", the ")}<a className="text-white underline underline-offset-4" href="https://policies.google.com/privacy" target="_blank" rel="noreferrer">{pick("Aviso de Privacidad de Google", "Google Privacy Policy")}</a>{pick(" y a los ", " and the ")}<Link href="/terminos" className="text-white underline underline-offset-4">{pick("Términos de Mexico Charts", "Mexico Charts Terms")}</Link>.</p>
            <p>{pick(`Para preguntas sobre privacidad o el uso de los servicios de la API de YouTube, contáctanos en ${CONTACT_EMAIL}.`, `For questions about privacy or the use of YouTube API Services, contact us at ${CONTACT_EMAIL}.`)}</p>
          </div>
        </section>
        ...
        <p className="mt-8 max-w-3xl text-xs font-medium leading-6 text-zinc-700">
          {pick("Esta política puede actualizarse conforme el sitio evolucione. Última actualización: 30 de agosto de 2026.", "This policy may be updated as the site evolves. Last updated: August 30, 2026.")}
        </p>
      </div>
