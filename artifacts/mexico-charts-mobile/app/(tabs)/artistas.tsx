...
function InitialAvatar({ name, size, accent }: { name: string; size: number; accent?: string }) {
  return (
    <View style={{
      ...
      borderWidth: 1, borderColor: accent ? `${accent}55` : "rgba(255,255,255,0.12)",
    }}>
      <Text style={{
        color: accent ?? "#E4E4E7",
...
function ArtistCard({ artist, photo }: { artist: Artist; photo: string | null }) {
  ...
  return (
    ...
    >
      ...
      <View style={styles.info}>
        ...
        <Text style={styles.tag} numberOfLines={1}>{artist.tag}</Text>
      </View>

      {/* Stats */}
      <View style={styles.statsCol}>
        <Text style={[styles.streams, { color: isTop3 ? artist.accent : "#A1A1AA" }]}>
          {artist.streams}
        </Text>
    ...
    <View style={[styles.container, { paddingTop: topInset }]}>
      ...
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.headerSub}>{TOP_ARTISTS.length} ARTISTAS · MÉXICO</Text>
        </View>
        <View style={styles.headerBadge}>
          <Feather name="users" size={12} color={NEON} />
          <Text style={styles.headerBadgeText}>BASE DE DATOS</Text>
        </View>
      </View>
      ...
      >
        {GENRE_FILTERS.map((f, i) => (
          ...
          >
            <Text style={[styles.pillText, i === activeFilter && styles.pillTextActive]}>
              {f.label}
            </Text>
          </TouchableOpacity>
        ))}
      ...
      <View style={styles.colHeaders}>
        ...
        <Text style={[styles.colHeader, { flex: 1, marginLeft: 52 }]}>ARTISTA</Text>
        <Text style={[styles.colHeader, { textAlign: "right", marginRight: 24 }]}>STREAMS</Text>
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <ArtistCard artist={item} photo={imageMap[item.name] ?? null} />
        )}
        ...
        ListEmptyComponent={
          <View style={{ paddingTop: 60, alignItems: "center" }}>
            ...
            <Text style={{ color: "#52525B", fontFamily: "Inter_500Medium", fontSize: 14, marginTop: 12 }}>
              Sin resultados
            </Text>
          </View>
        }
...
const styles = StyleSheet.create({
  ...
  headerSub: {
    color: "#52525B", fontFamily: "Inter_500Medium", fontSize: 10, letterSpacing: 1.5, marginTop: 2,
  },
  headerBadge: {
  ...
  pillText: {
    color: "#71717A", fontFamily: "Inter_600SemiBold",
  ...
  colHeader: {
    color: "#3F3F46", fontFamily: "Inter_700Bold",
  ...
  rank: { fontFamily: "Inter_700Bold", fontSize: 13, width: 24, letterSpacing: -0.5 },
  photo: { width: 48, height: 48, borderRadius: 24 },
  info: { flex: 1 },
  name: { color: "#E4E4E7", fontFamily: "Inter_600SemiBold", fontSize: 14 },
  tag: { color: "#52525B", fontFamily: "Inter_400Regular", fontSize: 10, marginTop: 2, letterSpacing: 0.5 },
