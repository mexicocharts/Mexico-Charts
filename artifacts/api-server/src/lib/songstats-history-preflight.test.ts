...
test("the compact store has no mutation target outside the five-table compact contract", () => {
  const source = readFileSync(
    new URL("./songstats-history-store.ts", import.meta.url),
    "utf8",
  );
  const mutationTargets = [
    ...source.matchAll(
      /\b(?:INSERT\s+INTO\s+|UPDATE\s+(?!SET\b)|CREATE\s+TABLE\s+IF\s+NOT\s+EXISTS\s+)([a-z_]+)/gi,
    ),
  ].map((match) => match[1]!.toLowerCase());
  assert.ok(mutationTargets.length > 0);
  assert.deepEqual(
    [
      ...new Set(
        mutationTargets.filter(
          (target) =>
            !(SONGSTATS_COMPACT_HISTORY_TABLES as readonly string[]).includes(
              target,
            ),
        ),
      ),
