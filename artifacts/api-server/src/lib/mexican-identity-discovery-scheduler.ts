...
export async function runScheduledMexicanIdentityDiscovery() {
  ...
  try {
    ...
    const prior = await client.query("SELECT 1 FROM mexican_identity_discovery_runs WHERE run_date = $1", [date]);
    if ((prior.rowCount ?? 0) > 0) return { date, status: "already_complete" as const };
    const limit = Math.max(1, Number(process.env["MEXICAN_IDENTITY_DAILY_LIMIT"] ?? 40));
