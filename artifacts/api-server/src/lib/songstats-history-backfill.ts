...
import {
  ...
  SongstatsApiError,
  type SongstatsHistoricStatsResponse,
...
function errorCode(error: unknown): string {
  return error instanceof SongstatsApiError
...
export async function runSongstatsHistoryBackfill(
  ...
  const tasks = roster.flatMap(artist => windows.map(window => ({ artist, window })));
  let nextTask = 0;
  ...
  const fetchHistoricStats = options.fetchHistoricStats ?? (async (artist, window) => {
    ...
    return getSongstatsArtistHistoricStats(
