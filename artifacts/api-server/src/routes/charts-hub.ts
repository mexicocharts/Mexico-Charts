...
router.get("/charts/editorial/weekly", async (req, res) => {
  try {
    await ensureOfficialChartArchive();
    const { pool } = await import("@workspace/db");
    const requestedDate = typeof req.query["date"] === "string" ? req.query["date"] : null;
    if (requestedDate && !/^\d{4}-\d{2}-\d{2}$/.test(requestedDate)) {
      res.status(400).json({ error: "Invalid edition date" }); return;
    }
    const chartKeys = [
      "Spotify_Artists_Weekly", "Spotify_Regional_Weekly",
      "YT_Artists_Weekly", "YT_Songs_Weekly",
      "Apple_Albums", "Apple_Songs", "Deezer_Top_Mexico",
    ];
    ...
    const availableDates = [...new Set(charts.map(chart => chart.chartDate))].sort().reverse();
    res.json({ generatedAt: new Date().toISOString(), requestedDate, editionDate: availableDates[0] ?? null,
      methodology: "Exact rank movement between saved official chart editions; no estimated positions.", charts });
  } catch (error) {
